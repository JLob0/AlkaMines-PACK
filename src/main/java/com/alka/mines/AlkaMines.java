...
public final class AlkaMines extends AlkaPlugin {
    ...
    protected void onPluginEnable() {
        ...
        this.shopHookSupplier = shopHookRef::get;
        this.dropHookSupplier = dropHookRef::get;

        getServer().getPluginManager().registerEvents(
                new MineBreakListener(mineManager, playerDataManager, levelManager, privateMineManager,
                        shopHookSupplier, mcmmoHookRef::get, aeHookRef::get, dropHookSupplier), this);
        ...
        // ===== Modulo de habilidades (dragao) - apos a inicializacao existente =====
        this.mineCache = new MineCache();
        for (Mine m : mineManager.getMines()) {
            mineCache.loadMine(m.getId(), m.getRegion(), m.getComposition());
        }
        this.taskManager = new TaskManager();
        this.fakeEntityRegistry = new FakeEntityRegistry();
        this.sessionManager = new MineAbilitySessionManager(taskManager, fakeEntityRegistry);
        getServer().getPluginManager().registerEvents(fakeEntityRegistry, this);
        getServer().getPluginManager().registerEvents(sessionManager, this);
        playerCommands.attachAbilityModule(mineCache, sessionManager, taskManager, fakeEntityRegistry,
                shopHookSupplier, dropHookSupplier);
    ...
    public MineAbilitySessionManager getSessionManager() {
